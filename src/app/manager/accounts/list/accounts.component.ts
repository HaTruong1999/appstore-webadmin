...
export class AccountsComponent implements OnInit {
  @ViewChild(AccountsCreateComponent) createModal;
  ...
  constructor(
    ...
    this.authService.checkMenu('accounts');
  ...
  getData()
  {
    ...
    this.accountsService.Accounts(this.currentPage, this.currentPageSize, this.searchValue, this.custId, this.sort)
    .subscribe((res: any) => {
  ...
  delete(posId: string){
    this.modal.confirm({
      ...
      nzOkType: 'primary',
      nzOkDanger: true,
      nzOnOk: () => this.submitDelete(posId),
      nzCancelText: this.translate.instant('global_cancel'),
    });
  }
  submitDelete(posId: string){
    this.accountsService.Delete(posId)
    .subscribe((res: any) => {
      if(res.code == 1)
      {
  ...
  exportExcel() {
    ...
    this.listData.forEach(item => {
      const { custId, accId, accCreatedby, accCreateddate, accIsactive, accPassword, accUpdatedby, accUpdateddate, userId, wpId, ...result } = item;
