...
export class RolesComponent implements OnInit {
  @ViewChild(RolesCreateComponent) createModal;
  ...
  constructor(
    ...
    this.authService.checkMenu('roles');
  ...
  getData()
  {
    ...
    this.rolesService.Roles(this.currentPage, this.currentPageSize, this.searchValue, this.sort)
    .subscribe((res: any) => {
  ...
  submitDelete(posId: string){
    this.rolesService.Delete(posId)
    .subscribe((res: any) => {
      if(res.code == 1)
      {
  ...
  exportExcel() {
    ...
    this.listData.forEach(item => {
      const { custId, roleId, roleCreatedby, roleCreateddate, ...result } = item;
